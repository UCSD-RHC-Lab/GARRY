...
that utilizes Human-Robot Interaction (HRI) and machine learning concepts.
...
To envision the motion capture process and early feedback design here is an early visualization:

[![Prototype Data Collection](https://img.youtube.com/vi/5qx3AhJqQp0/0.jpg)](https://www.youtube.com/watch?v=5qx3AhJqQp0)


The data is then downloaded and streamed into our app where patients can view the progress of a rehabilitation session in real-time as they are given different kinds of feedback on the quality of their step in relation to the predetermined goal metrics.

This data is collected by UC Davis researchers and sent to us for use in the system. They work with people recovering from stroke , which serves as the basis of the bulk of work. Our
...
#### Software
This system can be achieved with multiple devices as long as all are on the same network.
...
2. Open a terminal and run `pip install -r requirements.txt` to install Flask and flask-cors dependencies.


#### MATLAB
The folder "GARRY-MatlabWebsocket" should be placed on the Windows computer/laptop. Follow the below steps to set up.
...
7. Click on "Add Folder" and add the `src` and `examples` subfolders under the "GARRY-MatlabWebsocket" folder. Then click "Save". If it asks you if you want to save it to another location, hit "Yes". Lastly, hit "Close".
8. Run `server = TestServer(4000)` and verify that there is no errors. If this does not work, restart MATLAB and try again.

#### Flutter
The folder "GARRY-Flutter" should be placed on the Windows computer/laptop.
1. Download the Flutter SDK any version >= 3.7.12 from [here](https://urldefense.com/v3/__https://docs.flutter.dev/release/archive?tab=windows__;!!Mih3wA!FPQOotNdzvKmibdlCGmC0sRnAx04wmr_9Q5h-U32R1s7nbmaKpWulAA1Tk9utBAhqtewc5KVmnAE2W3g$ ). It will likely be a zip file.
...
#### ROS/Turtlebot
1. Move our `garry_ros` into the `src` subfolder in your catkin workspace.
2. Run `cd ..` back to your root `catkin_ws` directory and run `catkin make`.
...
In the future, we will separate this into two files: `sessions_api.dart` and `participants_api.dart`.
